% 
% Implementation of SKH (Simulated annealing based krill herd algorithm)
%
% Author:
%   Dhananjay Bhaskar
%
...
function [minima, minimizer] = SKH(xmin, xmax, fun, dim)
    ...
    Bconst = 1;     % Boltzmann constant
    schedule = 0.95;% cooling factor
    thres = 0.01;   % acceptance threshold
    ...
    while (t < MaxGen)
            ...
            for i = 1 : N
                ...
                omegaInduced = 0.9;
                alpha = calculateAlphaLoc(x, dim, i, N) + calculateAlphaTarget(x, dim, i, t, MaxGen);
                ...
                f_new = feval(fun, xnew(i,:));
                
                temperature = schedule * temperature;               % cooling
                
                % simulated annealing
                deltaF = f_new - x(i,dim+1);
                
                if( -1*deltaF > thres)                              % accept improved solution
                    x(i, 1:dim) = xnew(i,:);
                else
                    r = rand(1);
                    acceptance_prob = exp(((-1*deltaF)/(Bconst*temperature)));
                    if ( (deltaF <= thres) && (acceptance_prob > r) )           % accept bad solutions with a low probability
                        x(i, 1:dim) = xnew(i,:);
                    end
                end
...
function [alphatarget] = calculateAlphaTarget(x, dim, i, t, MaxGen)
